...
import { strict_output } from "./openaiAction";


const SearchYoutubeAction = 
     async(searchQuery:string) => {
            searchQuery = encodeURIComponent(searchQuery);
            const { data } = await axios.get(
              `https://www.googleapis.com/youtube/v3/search?key=${"AIzaSyBDd_cBq8Hu3md7cbXgWFWBkq4qL9awCiE"}&q=${searchQuery}&videoDuration=medium&videoEmbeddable=true&type=video&maxResults=5&relevanceLanguage=en`
            );
            if (!data) {
 ...
 const getTranscriptFromVideoId=
...
export const getFinalSummary=action({
